...
const mockClothing: Record<string, ClothingItem> = {
  "1": {
    id: "1",
...
export async function generateMetadata({ params }: { params: { id: string } }) {
  const item = mockClothing[params.id];
  if (!item) return { title: "Item Not Found | ACME Store" };
  return {
...
export default function ClothingItemPage({ params }: { params: { id: string } }) {
  const item = mockClothing[params.id];
  if (!item) notFound();
